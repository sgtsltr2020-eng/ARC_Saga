...
@app.post("/capture")
async def capture_message(request: Request, body: CaptureRequest):
    ...
    validate_capture_request(
        ...
        metadata_keys=(body.metadata or {}).keys(),
    )
    ...
    provider = provider_map.get(body.source.lower(), Provider.OPENAI)

    # Map role string to MessageRole enum
    role = MessageRole.USER if body.role.lower() == "user" else MessageRole.ASSISTANT
    ...
    message = Message(
        ...
        session_id=body.thread_id,  # thread_id maps to session_id
    ...
    try:
        ...
        # Auto-tag
        tags = auto_tagger.extract_tags(body.content)

        return {
            "message_id": message_id,
            "thread_id": body.thread_id or message.session_id,
            "tags": tags,
            "status": "stored"
        }
    except Exception as e:
...
@app.post("/search")
async def search_memory(request: Request, body: SearchRequest):
    """Search across all conversations"""

    validate_search_request(query=body.query or "", limit=body.limit)

    try:
        # Handle empty query for "recent" search
        query = body.query.strip() if body.query else "test"
...
async def ask_perplexity(request: Request, body: PerplexityRequest):
    """Ask Perplexity with automatic context injection"""

    validate_perplexity_request(query=body.query)
    ...
    context_messages = []
    if body.inject_context and body.thread_id:
        messages = await storage.get_by_session(body.thread_id)
        context_messages = [
